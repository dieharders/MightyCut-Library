...
export const StatSchema = z.object({
  ...
  suffix: z.string().max(10).optional().describe('Trailing unit, e.g. "%", "x", "hrs"'),
  decimals: z.number().int().min(0).max(2).default(0).describe("Decimal places shown while counting up"),
  accent: z.enum(ACCENTS).default("pink").describe("Accent color token for the corner dot"),
});
